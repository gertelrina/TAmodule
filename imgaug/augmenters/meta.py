    ...
    def _augment_batch_(self, batch, random_state, parents, hooks):
        with batch.propagation_hooks_ctx(self, hooks, parents):
            ...
            augmenter_active = self._get_augmenter_active(batch.nb_rows,
                                                          random_state)
            # print('augmenter_active', augmenter_active)

            for augmenter_index in augmenter_order:
                # print('augmenter_index', augmenter_index)
                active = augmenter_active[:, augmenter_index].nonzero()[0]
                # print('active', active)
                if len(active) > 0:
                    # print('parents', parents)
                    print('aug_name - ', self[augmenter_index].name) 
    ...
    **Supported dtypes**:
    ...
    def __init__(self, children,
        ...
        super(OneOf, self).__init__(
            ...
            seed=seed, name=name,
            random_state=random_state, deterministic=deterministic)
        # print(f'{type(children)}, children {children}')
